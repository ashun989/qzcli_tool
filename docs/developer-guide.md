...
2. 独立的 login 环境变量或 `~/.qzcli/config.json` 中已保存的 login 认证信息
3. 交互式输入

这里的 login 认证信息与 `init` 使用的 OpenAPI 用户名密码分开存储，避免两套密码混用。

Cookie 保存在 `~/.qzcli/.cookie`，其中还可以附带默认 `workspace_id`。

### 4.3 全局代理配置

项目支持单一的全局代理 URL，来源优先级为：

1. 环境变量 `QZCLI_PROXY_URL`
2. `~/.qzcli/config.json` 中的 `proxy_url`
...
其中 `socks5://` 在运行时会被转换为 `socks5h://` 供 `requests` 使用，以便由代理端完成域名解析。

代理一旦配置，会统一覆盖：

- OpenAPI 请求
- 内部 `/api/v1/*` 请求
- `login` 命令中的 CAS / Keycloak / 启智站点登录流程
...
- `resources.json`
  ...
  - 包含项目、计算组、规格、更新时间。
- `retry_configs/`
  - `retry-watch` 自动保存的失败任务配置快照。
- `jobs.json`
  - 本地活跃追踪的任务列表。
- `jobs.archive.jsonl`
  - 被 `qzcli prune` 归档的历史任务。
...
- 跟踪外部脚本创建的任务 ID。
- 通过 `qzcli ls -c --all-ws --track` 批量同步平台上当前可见的任务。
- 轮询刷新任务状态。
- 输出更友好的本地视图。
- 通过 `qzcli prune` 将超过 TTL 的完成态任务归档出主列表。
...
- `prune`
  - 只依赖本地 `jobs.json` / `jobs.archive.jsonl`，不调用远端接口。
- `create`
  - 依赖 OpenAPI `train_job/create`。
- `retry-watch`
  - 单次调用 `train_job/detail` 检查单任务状态，不做阻塞轮询。
  - 按任务名启动时，先通过内部接口 `/api/v1/train_job/list` 解析候选任务；只有唯一命中时才继续。
  - 如果查询时发现 cookie 失效，会尝试复用已保存的 login 凭证自动调用 CAS 登录并刷新 cookie，再重试一次任务列表查询。
  - 如果按任务名命中多条同名任务，会输出候选列表；交互终端中允许按编号直接选择，非交互环境则终止并要求显式指定 `--job-id`。
  - 如果当前任务仍处于运行/排队态，会把该任务登记为本地监控链的最新叶子节点；后续用根 `job_id` 再次执行时，会自动沿 `retry_submitted_job_id` 跳到最新子任务继续检查。
  - 检测到 `job_failed` 后，把详情响应规整成可复用的 create payload，落盘到 `retry_configs/`，再调用 OpenAPI `train_job/create` 自动重提。
  - 已处理过的失败任务会在本地 `jobs.json` 中打标，避免同一个 `job_id` 被重复重提。
- `list -c`
  - 依赖内部接口 `/api/v1/train_job/list`。
- `list -c --track`
  - 在 `list -c` 拉取平台任务后，批量同步到本地 `jobs.json`。

这里的设计是：

- “任务详情/停止”走官方接口。
- “任务创建”通过 JSON 文件直接透传到官方 OpenAPI。
- “任务列表”优先走内部接口，因为它能按工作空间拉取更多上下文。
- `track <job_id>` 用于单任务补录，`list -c --track` 用于批量同步当前可见任务。
- `prune` 用于按 TTL 清理终态历史，避免 `watch` / 本地 `list` 长期只增不减。
...
`specs` 使用 OpenAPI `/openapi/v1/specs/list` 查询某个计算组可用规格。

CLI 层会先复用 `resources.json` 做工作空间和计算组名称解析，再按 `logic_compute_group_id` 调接口。
...
1. `workspace --sync` 分支看起来存在方法名不一致风险。
